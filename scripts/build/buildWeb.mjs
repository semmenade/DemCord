...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/DemCord.ts"],
    format: "iife",
    globalName: "DemCord",
...
const buildConfigs = [
    ...
    },
    {
        ...
        footer: { js: "//# sourceURL=file:///DemCordWeb" }
    },
    {
        ...commonOptions,
    ...
    },
    {
        ...
        outfile: "dist/DemCord.user.js",
        banner: {
            js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
        },
        footer: {
            // UserScripts get wrapped in an iife, so define DemCord prop on window that returns our local
            js: "Object.defineProperty(unsafeWindow,'DemCord',{get:()=>DemCord});"
        }
    }
];
...
async function buildExtension(target, files) {
    const entries = {
        "dist/DemCord.js": await readFile("dist/extension.js"),
        "dist/DemCord.css": await readFile("dist/extension.css"),
...
const appendCssRuntime = readFile("dist/DemCord.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/DemCord.user.js", cssRuntime);
});
