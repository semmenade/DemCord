...
function ExcludedPluginsList({ search }: { search: string; }) {
    ...
    const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "desktop" | "dev", string> = {
        ...
        dev: "Developer version of DemCord"
    };
...
function PluginSettings() {
    ...
    const [newPlugins] = useAwaiter(() => DataStore.get("DemCord_existingPlugins").then((cachedPlugins: Record<string, number> | undefined) => {
        ...
        DataStore.set("DemCord_existingPlugins", existingTimestamps);
    ...
    for (const p of sortedPlugins) {
        ...
        if (isRequired) {
            const tooltipText = p.required || !depMap[p.name]
                ? "This plugin is required for DemCord to function."
    ...
    return (
        <SettingsTab>
            ...
            <UIElementsButton />{!IS_WEB && <Button onClick={() => VencordNative.native.openExternal("file:///" + VencordNative.getSettingsDir() + "/userplugins")} size={Button.Sizes.SMALL} color={Button.Colors.PRIMARY} style={{marginTop:"8px"}}>Import Plugin (.ts/.tsx)</Button>}
