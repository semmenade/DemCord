...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => DemCord.WebpackPatcher },
        wpInstances: { getter: () => DemCord.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => DemCord.Plugins },
        plugins: { getter: () => DemCord.Plugins.plugins },
        Settings: { getter: () => DemCord.Settings },
        Api: { getter: () => DemCord.Api },
        Util: { getter: () => DemCord.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("DemCord-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (DemCord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$DemCordGetWrappedComponent) {
            return forceLoad ? value.$$DemCordGetWrappedComponent() : value;
        }
